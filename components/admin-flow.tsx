    ...
    <section className="mb-5 border border-line bg-surface-elevated-2 p-5">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="max-w-4xl">
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-normal md:text-3xl">{title}</h2>
    ...
    >
      <span>
        <span className="flex items-center justify-between gap-3">
          ...
          {badge ? (
            <ClearStateBadge tone={tone === "warn" ? "warn" : "info"}>{badge}</ClearStateBadge>
          ) : null}
        </span>
        <span className="mt-4 block text-[0.68rem] font-bold uppercase text-muted">{label}</span>
      ...
      <div className="mt-4 grid gap-2">
        {items.map((item) => (
          ...
          >
            ...
            <span className={clsx("truncate font-semibold", railTone(item.tone ?? "neutral"))}>
              {item.value}
            </span>
          </div>
        ))}
