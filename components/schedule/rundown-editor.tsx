...
export function RundownEditor({
    ...
    function moveByButton(id: string, delta: number) {
        ...
        if (newIndex < 0 || newIndex >= orderedIds.length) {
            return;
        }
        const nextIds = arrayMove(orderedIds, oldIndex, newIndex);
        run(
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-4 py-3">
                <div>
                    ...
                    <p className="mt-1 text-sm text-muted">
                        Drag reorder, resize, duplicate, archive and bulk status for the programming
                        day.
                    </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                        ...
                        onClick={() =>
                            run(() =>
                                bulkStatusAction({ blockIds: selectedIds, status: bulkStatus }),
                            )
                        }
                    >
                    ...
                    <SortableContext items={orderedIds} strategy={verticalListSortingStrategy}>
                        {orderedBlocks.map((block, index) => (
                            <SortableRundownRow
                                ...
                                onDuplicate={() =>
                                    run(() => duplicateAction({ blockId: block.id }))
                                }
