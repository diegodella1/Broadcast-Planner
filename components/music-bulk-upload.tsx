  ...
  async function uploadRows() {
    ...
    try {
      ...
      for (const row of pendingRows) {
        setRows((current) =>
          current.map((item) =>
            item.id === row.id ? { ...item, status: "uploading", message: "Uploading..." } : item
          )
        )
        try {
          ...
          setRows((current) =>
            current.map((item) =>
              item.id === row.id ? { ...item, status: "uploaded", message: "Uploaded" } : item
            )
          )
        } catch (error) {
          setRows((current) =>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            Select MP3 files, review detected metadata, then upload them into the background
            playlist.
          </p>
        </div>
            ...
            {rows.map((row) => (
              ...
              >
                ...
                <input
                  value={row.album}
                  placeholder="Album"
                  onChange={(event) => updateRow(row.id, { album: event.target.value })}
                  className="border border-line px-2 py-1.5"
                />
                <input
                  ...
                  min="1"
                  placeholder="Sec"
                  onChange={(event) => updateRow(row.id, { durationSeconds: event.target.value })}
                  className="border border-line px-2 py-1.5"
                />
...
async function readId3Tags(file: File): Promise<MusicMetadata> {
  ...
  if (bytes.length < 10 || text(bytes, 0, 3) !== "ID3") return {}
  ...
  while (offset + 10 <= limit) {
    ...
    if (id === "TIT2") tags.music_title = value
    if (id === "TPE1") tags.artist = value
    if (id === "TALB") tags.album = value
    if (id === "TYER" || id === "TDRC") tags.year = value
    if (id === "TRCK") tags.track = value
    if (id === "TCON") tags.genre = value
    offset += 10 + size
  }
  return tags
}
...
async function fetchFreshCsrfToken() {
  ...
  if (!response.ok || !data.csrfToken) throw new Error("Could not refresh CSRF token")
