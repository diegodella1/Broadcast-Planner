    ...
    useEffect(() => {
        ...
        async function run() {
            ...
            try {
                ...
                if (!r.ok) {
                    throw new Error(`Reuters channels failed: ${r.status}`);
                }
    ...
    function syncReuters() {
        ...
        void (async () => {
            try {
                ...
                if (!r.ok) {
                    throw new Error(`Reuters sync failed: ${r.status}`);
                }
                const data = (await r.json()) as { channels: ReutersChannelRow[] };
                setReutersChannels(data.channels ?? []);
            } catch (e: unknown) {
        ...
        <div className="space-y-2">
            ...
            {source === 'vimeo' ? (
                <>
                    ...
                    {vimeoSearchError && (
                        <p className="text-[10px] text-negative-red">{vimeoSearchError}</p>
                    )}
                ...
                <>
                    ...
                    {isLoadingReuters && (
                        <p className="text-[10px] text-white/40">{t('searching')}</p>
                    )}
                    {reutersError && (
                        <p className="text-[10px] text-negative-red">{reutersError}</p>
                    )}
                    ...
                    >
                        ...
                        {!isLoadingReuters && filteredReuters.length === 0 && !reutersError && (
                            <li className="text-xs text-white/40 px-2 py-1">
                                {t('noReutersConfigured')}
                            </li>
                        )}
                    </ul>
                </>
