...
if (onlyInEn.length) {
    console.error(`Only in en.json (${onlyInEn.length}): ${onlyInEn.join(', ')}`);
}

if (onlyInEs.length) {
    console.error(`Only in es.json (${onlyInEs.length}): ${onlyInEs.join(', ')}`);
}

if (empties.length) {
    console.error(`Empty values (${empties.length}): ${empties.join(', ')}`);
}

if (nonStrings.length) {
    console.error(`Non-string values (${nonStrings.length}): ${nonStrings.join(', ')}`);
}
process.exit(1);
