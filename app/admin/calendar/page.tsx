...
import { createDaySchema, parseFormData } from "@/lib/schemas"
...
export default async function CalendarPage() {
  ...
  async function createDay(formData: FormData) {
    "use server"
    const parsed = parseFormData(createDaySchema, {
      date: formData.get("date")
    })
    await ensureProgramDay(parsed.date)
  }
  return (
    <AdminShell
      title={t("title")}
      description={t("description")}
      actions={<ButtonLink href={`/admin/schedule/${today}`}>{t("scheduleToday")}</ButtonLink>}
    >
      <section className="mb-5 grid gap-3 md:grid-cols-3">
        <MetricTile
          label={t("metrics.days")}
          value={String(days.length)}
          detail={t("metrics.daysDetail")}
        />
        <MetricTile
