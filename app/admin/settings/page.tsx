...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ saved?: string; imported?: string; show_uri?: string }> }) {
  ...
  return (
    <AdminShell title="Integrations" description="Vimeo credentials, operating timezone and import tools.">
      ...
      <form action="/rtvtime/api/settings" method="post" className="surface-panel max-w-2xl p-5">
        <FormHeader title="Vimeo integration" detail="Secrets are encrypted server-side and never returned in full from the API." />
        <div className="mt-4 rounded-md bg-panel-soft px-3 py-2 text-sm text-muted">
          Vimeo: {settings?.hasSecret ? `token saved (${settings.status})` : "no token saved"}
          {settings?.lastError ? <span className="block text-danger">Last error: {settings.lastError}</span> : null}
        </div>
      ...
      <section className="surface-panel mt-6 max-w-2xl p-5">
        <FormHeader title="Vimeo shows and episodes" detail="Pick a show and convert visible episodes into reviewable assets." />
