...
export default async function SettingsPage({ searchParams }: { searchParams: Promise<{ saved?: string; imported?: string; show_uri?: string }> }) {
  ...
  return (
    <AdminShell title="Settings" description="Credenciales, timezone operativo e importacion Vimeo.">
      {params.saved ? <Notice tone="ok">Settings guardados.</Notice> : null}
      {params.imported ? <Notice tone="ok">Episodio importado como asset.</Notice> : null}
      <form action="/rtvtime/api/settings" method="post" className="surface-panel max-w-2xl p-5">
        <FormHeader title="Integraciones" detail="Los secretos se guardan cifrados server-side y no se devuelven completos desde la API." />
        <div className="mt-4 rounded-md bg-panel-soft px-3 py-2 text-sm text-muted">
          Vimeo: {settings?.hasSecret ? `token guardado (${settings.status})` : "sin token guardado"}
          {settings?.lastError ? <span className="block text-danger">Ultimo error: {settings.lastError}</span> : null}
        </div>
        <label className="mt-5 block text-sm font-medium">
          Vimeo API token
          <input name="vimeo_token" type="password" className="mt-2 w-full border border-line px-3 py-2" placeholder="Bearer token" />
        </label>
        <label className="mt-4 block text-sm font-medium">
          Vimeo folder/project URI opcional
        ...
        <button className="btn-primary mt-5">Guardar settings</button>
      </form>

      <section className="surface-panel mt-6 max-w-2xl p-5">
        <FormHeader title="Vimeo shows y episodios" detail="Elegí un show y convertí episodios visibles en assets listos para revisar." />
        {!token ? <p className="mt-3 text-sm text-muted">Guardá un Vimeo API token para listar shows.</p> : null}
            ...
            <form action="/rtvtime/admin/settings" className="mt-4 grid gap-3">
              <label className="block text-sm font-medium">
                ...
                <select name="show_uri" defaultValue={selectedShowUri} className="mt-2 w-full border border-line px-3 py-2">
                  <option value="">Elegir show</option>
              ...
              <button className="btn-secondary w-fit">Ver episodios</button>
            </form>
            {selectedShowUri ? <EpisodePicker episodes={vimeoState.episodes} showUri={selectedShowUri} /> : null}
          </>
        ) : null}
      </section>
    </AdminShell>
...
function EpisodePicker({ episodes, showUri }: { episodes: VimeoVideo[]; showUri: string }) {
  if (!episodes.length) {
    return <p className="mt-4 text-sm text-muted">Ese show no tiene episodios visibles para este token.</p>
  }
  return (
    <form action="/rtvtime/api/vimeo/import" method="post" className="mt-5 grid gap-3 rounded-md bg-panel-soft p-4">
      ...
      <button className="btn-primary w-fit">Importar episodio como asset</button>
    </form>
  )
}
