...
export default async function AssetsPage({
  ...
  async function addAsset(formData: FormData) {
    "use server"
    const data = parseFormData(createMediaAssetSchema, {
  ...
  type BuildHrefOverrides = Partial<SearchParams> & { clearStatus?: boolean; clearKind?: boolean }
  function buildHref(overrides: BuildHrefOverrides): string {
    const { clearStatus, clearKind, ...rest } = overrides
    const baseStatus = clearStatus ? undefined : (rest.status ?? params.status)
    const baseKind = clearKind ? undefined : (rest.kind ?? params.kind)
    ...
    >
      ...
      <section className="mb-5 grid gap-3 md:grid-cols-3">
        <MetricTile
          ...
          detail={t("metrics.totalDetail")}
        />
        <MetricTile
          label={t("metrics.ready")}
          value={String(readyCount)}
          detail={t("metrics.readyDetail")}
          tone="ok"
        />
        <MetricTile
          label={t("metrics.review")}
          value={String(attentionCount)}
          detail={t("metrics.reviewDetail")}
        ...
        >
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_170px_140px_120px]">
            <input
              name="title"
              required
              placeholder={t("uploadVideo.videoTitle")}
        ...
        <form action={addAsset} className="surface-panel p-4">
          ...
          <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_1fr_130px]">
            <input
              name="title"
              required
              placeholder={t("addUrl.title2")}
              className="border border-line px-3 py-2 text-sm"
            />
            <input
              name="url"
              placeholder={t("addUrl.url")}
              className="border border-line px-3 py-2 text-sm"
            />
            <input
              name="duration_seconds"
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <FilterLink
          href={buildHref({ clearStatus: true, clearKind: true })}
          active={!params.status && !params.kind}
        >
          {t("tabs.all")}
        </FilterLink>
        <FilterLink
          href={buildHref({ status: "attention", clearKind: true })}
          active={params.status === "attention"}
        >
          {t("tabs.review")}
        </FilterLink>
        <FilterLink
          href={buildHref({ status: "ready", clearKind: true })}
          active={params.status === "ready"}
        >
          {t("tabs.ready")}
        </FilterLink>
        <FilterLink
          href={buildHref({ kind: "fallback", clearStatus: true })}
          active={params.kind === "fallback"}
        >
          {t("tabs.fallbacks")}
        </FilterLink>
        <FilterLink
          href={buildHref({ kind: "ad", clearStatus: true })}
          active={params.kind === "ad"}
        >
          {t("tabs.ads")}
        </FilterLink>
        <FilterLink
          href={buildHref({ kind: "promo", clearStatus: true })}
              ...
              <summary className="grid cursor-pointer list-none gap-3 md:grid-cols-[1fr_150px_140px_120px_90px] md:items-center">
                <div>
                  ...
                  <p className="text-sm text-muted">
                    ...
                    {asset.metadata?.presentation === "vertical_blur"
                      ? ` · ${t("verticalBlur")}`
                      : ""}
                  </p>
                </div>
