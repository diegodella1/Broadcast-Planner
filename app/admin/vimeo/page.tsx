  ...
  return (
    ...
    >
      ...
      <section className="mb-5 grid gap-3 md:grid-cols-4">
        ...
        />
        <MetricTile label="Ready" value={String(readyCount)} detail="Playable now" tone="ok" />
        <MetricTile
          label="Review"
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted">
              Last sync: {lastSyncAt || "never"} · Last count: {lastSyncCount || "0"} · Shows in
      ...
      <section className="mb-4 rounded-lg border border-line bg-surface p-3">
        ...
        <div className="flex flex-wrap gap-2">
          <FilterLink href="/admin/vimeo" active={!params.status && !params.q && !params.show_name}>
            All Vimeo
          </FilterLink>
          <FilterLink href="/admin/vimeo?status=ready" active={params.status === "ready"}>
            Ready
          </FilterLink>
          <FilterLink href="/admin/vimeo?status=archived" active={params.status === "archived"}>
            Archived
          </FilterLink>
        </div>
      </section>
  ...
  return (
    <div className="grid gap-3 p-4 md:grid-cols-[120px_1fr_150px] md:items-center">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 text-sm text-muted">
          {showName || "No show"} · {asset.durationSeconds ?? 0}s · {formatVimeoDate(created)} ·{" "}
          {asset.status}
        </p>
...
function uniqueShowNames(assets: MediaAsset[]) {
  return [
    ...new Set(assets.map((asset) => getMetadataText(asset, "vimeo_show_name")).filter(Boolean))
  ].sort()
}
