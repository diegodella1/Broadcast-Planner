...
export default async function OperatorRunbookPage({
  ...
  async function saveCheck(formData: FormData) {
    ...
    if (!currentSchedule.day) throw new Error("Program day missing")
  ...
  const checkedCount = RUNBOOK_TEMPLATE.reduce(
    (total, section) =>
      total +
      section.items.filter((item) => stateByKey.get(`${section.section}:${item.key}`)?.checked)
        .length,
    0
  )
  ...
  const criticalOpen = RUNBOOK_TEMPLATE.flatMap((section) =>
    section.items
      .filter((item) => item.critical && !stateByKey.get(`${section.section}:${item.key}`)?.checked)
      .map((item) => item.label)
  )
        ...
        <>
          <section className="mb-5 grid gap-3 lg:grid-cols-3">
            <div className="surface-card p-4">
              ...
              <p className="mt-2 text-lg font-semibold">{schedule.day.title ?? date}</p>
              <div className="mt-2">
            ...
            <div className="surface-card p-4">
              ...
              <p className="mt-2 text-2xl font-semibold tabular-nums">{criticalOpen.length}</p>
          ...
          <div className="grid gap-5">
            {RUNBOOK_TEMPLATE.map((section) => (
              <section key={section.section} className="surface-panel overflow-hidden">
                <div className="border-b border-line px-4 py-3">
                  <h2 className="font-semibold">{section.title}</h2>
                </div>
                <div className="divide-y divide-line">
                  {section.items.map((item) => {
                    const current = stateByKey.get(`${section.section}:${item.key}`)
                    return (
                      <RunbookItemForm
                        key={item.key}
                        action={saveCheck}
