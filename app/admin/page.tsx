...
import { AlertTriangle, CalendarDays, CheckCircle2, Library, MonitorPlay, Music, Plus, RadioTower } from "lucide-react"
    ...
    <AdminShell
      ...
      actions={
        <>
          ...
          <ButtonLink href="/output/live?debug=true" variant="secondary">Open output</ButtonLink>
        </>
      }
    >
      <section className="mb-5 grid gap-3 xl:grid-cols-[1.25fr_1fr]">
        <div className="surface-panel p-5">
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            <SignalTile
              ...
              detail={active.block ? `${formatTimecode(active.elapsedInBlock)} / ${formatTimecode(active.block.durationSeconds)}` : active.reason ?? "No schedule window"}
            ...
            <SignalTile
              ...
              title={health.criticalCount ? `${health.criticalCount} critical` : health.warnCount ? `${health.warnCount} warnings` : "Clear"}
        ...
        <section className="surface-panel p-5">
          ...
          <div className="mt-4 grid gap-2">
            <ActionLink href={`/admin/schedule/${today}`} icon={<Plus size={17} />} title={blocks.length ? "Edit today's timeline" : "Create today's first block"} detail="Drag on the timeline, add media, validate timing." />
            <ActionLink href="/admin/assets?status=attention" icon={<Library size={17} />} title={reviewAssets ? "Review media assets" : "Media library clear"} detail={`${reviewAssets} assets need attention.`} />
            <ActionLink href="/admin/music" icon={<Music size={17} />} title={musicReady ? "Check music playlist" : "Add background music"} detail={`${musicReady} ready music tracks available.`} />
            <ActionLink href="/output/live?debug=true" icon={<MonitorPlay size={17} />} title="Open output monitor" detail="Verify renderer state, fallback and music debug." />
          </div>
        </section>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="surface-panel overflow-hidden">
          <div className="border-b border-line px-4 py-3">
            ...
            <p className="mt-1 text-sm text-muted">The next ready or active blocks from the current time.</p>
          </div>
          {blocks.length ? (
            ...
            <div className="p-4">
              <EmptyState title="No blocks today" action={<ButtonLink href={`/admin/schedule/${today}`}>Open timeline</ButtonLink>}>
        ...
        <section className="surface-panel p-4">
          ...
          <div className="mt-4 grid gap-2">
            {health.issues.slice(0, 8).map((issue) => (
              <Link
                key={issue.id}
                href={issue.blockId ? `/admin/schedule/${today}/blocks/${issue.blockId}` : `/admin/schedule/${today}`}
                className={issue.severity === "critical" ? "rounded-md border border-danger-line bg-danger-soft px-3 py-2 text-sm text-danger-strong" : "rounded-md border border-warn-line bg-warn-soft px-3 py-2 text-sm text-warn-strong"}
              >
            ...
            {!health.issues.length ? (
              <p className="rounded-md border border-success-line bg-success-soft px-3 py-2 text-sm text-success-strong">No schedule issues detected today.</p>
            ) : null}
          </div>
        </section>
      </section>
...
function ActionLink({ href, icon, title, detail }: { href: string; icon: ReactNode; title: string; detail: string }) {
  return (
    <Link href={href} className="flex gap-3 rounded-md border border-line bg-surface px-3 py-3 text-sm hover:bg-panel-soft">
