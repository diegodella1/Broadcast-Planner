    ...
    >
        ...
        <div className="surface-panel min-w-0">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line px-4 py-3">
            <div>
              ...
              <p className="mt-1 text-sm text-muted">Hourly blocks with proportional duration and continuity warnings.</p>
            </div>
            {health.issues.length ? (
              ...
              <span className="rounded-full border border-success-line bg-success-soft px-3 py-1 text-xs font-semibold text-success-strong">No alerts</span>
            )}
          </div>
          <ScheduleTimeline blocks={blocks} schedule={schedule} date={date} nowSeconds={isToday ? nowSeconds : null} issues={health.issues} createBlockAction={addBlock} />
        </div>
        ...
        <aside className="grid gap-5 content-start">
          <section className="surface-panel p-4">
            ...
            <div className="mt-4 grid gap-3">
              ...
              <Metric label="Overlaps" value={String(health.overlaps.length)} tone={health.overlaps.length ? "danger" : "ok"} />
              <Metric label="Missing assets" value={String(health.missingAssets.length)} tone={health.missingAssets.length ? "danger" : "ok"} />
              <Metric label="Unready assets" value={String(health.unreadyAssets.length)} tone={health.unreadyAssets.length ? "warn" : "ok"} />
            </div>
            <div className="mt-4 grid gap-2">
              ...
              {health.issues.length === 0 && <p className="rounded-md bg-panel-soft px-3 py-2 text-sm text-muted">No conflicts detected for this schedule.</p>}
            </div>
          </section>
          ...
          <details className="surface-panel p-4">
            ...
            <form action={addBlock} className="mt-4 grid gap-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <input name="pre_roll_seconds" type="number" min="0" defaultValue="0" placeholder="Before sec" className="border border-line px-3 py-2 text-sm" />
                <input name="post_roll_seconds" type="number" min="0" defaultValue="0" placeholder="After sec" className="border border-line px-3 py-2 text-sm" />
              </div>
              <p className="text-xs leading-5 text-muted">
                If the asset or slide has a known duration, save expands the block to at least content plus before/after time.
              </p>
      ...
      <section className="surface-panel mb-5 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <FormHeader title="Long grid generator" detail="Create consecutive blocks for long-duration tests with programs, ad breaks and image bumpers." />
          </div>
      ...
      <div className="surface-panel overflow-x-auto">
        ...
        {blocks.length === 0 ? <div className="p-4"><EmptyState title="No blocks scheduled">Add the first block or generate a long grid to test continuity.</EmptyState></div> : null}
        </div>
      </div>
    </AdminShell>
  )
