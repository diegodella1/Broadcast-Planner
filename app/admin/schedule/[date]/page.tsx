...
function Rundown({
  ...
  const nowLineIndex =
    nowSeconds !== null
      ? blocks.findIndex((block) => block.startTimeSeconds >= nowSeconds)
      : -1

  // If nowSeconds is past all blocks, append at end
  const nowAtEnd =
    nowSeconds !== null && nowLineIndex === -1 && blocks.length > 0
      ...
      {blocks.length === 0 ? (
        ...
        <div className="flex flex-col gap-2 pr-3">
          {blocks.map((block, index) => {
            ...
            const statusLabel =
              NON_DEFAULT_STATUSES.has(block.status) ? tBlockStatus(block.status) : null
