...
export async function createOperatorSession(input: {
  ...
  if (!handle) {
    if (!isAdminTokenValid(input.token)) return null
  ...
  if (!data?.id || !safeEqual(hashSecret(input.token), String(data.token_hash))) return null
...
export async function getCurrentOperatorSession(): Promise<OperatorSession | null> {
  try {
    ...
    const { data, error } = await supabase
      .from("admin_sessions")
      .select("id, expires_at, revoked_at, admin_operators(id, handle, display_name, role, status)")
      .eq("session_hash", hashSecret(sessionToken))
      .maybeSingle()
    if (error) throw error
    ...
    if (!operatorRow || String(operatorRow.status) !== "active") return null
    return {
