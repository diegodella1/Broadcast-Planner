...
export function parseReutersStreamInput(input: {
  ...
  if (!protocol) throw new Error("Reuters stream must be an HLS (.m3u8) or RTMP URL")
  return {
    protocol,
    url,
...
export function protocolForReutersUrl(url: string): "hls" | "rtmp" | null {
  ...
  if (/^https?:\/\/.+\.m3u8(?:[?#].*)?$/i.test(url)) return "hls"
  return null
}
...
function normalizeExpiry(value?: string | null) {
  ...
  if (!Number.isFinite(timestamp)) throw new Error("Reuters stream expiry is invalid")
