...
export function isRecordedBugPosition(value: unknown): value is RecordedBugPosition {
    return (
        typeof value === 'string' && RECORDED_BUG_POSITIONS.includes(value as RecordedBugPosition)
    );
}
...
export function recordedBugFromBlock(block: Pick<ProgramBlock, 'blockType' | 'metadata'>) {
    if (!isRecordedBugEligibleBlock(block)) {
        return null;
    }

    if (block.metadata?.previously_recorded_enabled !== true) {
        return null;
    }
...
export function recordedBugMetadata(input: {
    ...
    if (input.blockType !== 'video' || input.hasReutersStream || !input.enabled) {
        return next;
    }
