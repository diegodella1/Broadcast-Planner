...
async function fetchPyth(): Promise<{
  ...
  try {
    const ids = Object.values(PYTH_FEED_IDS).map((id) => (id.startsWith("0x") ? id.slice(2) : id))
    ...
    if (!response.ok) throw new Error(`Pyth Hermes error: ${response.status}`)
...
async function fetchCopperFromRtvApi(): Promise<CommodityRaw> {
  try {
    ...
    if (!response.ok) {
      console.warn(`[lib/slides/data/markets.ts:fetchCopperFromRtvApi] HTTP ${response.status}`)
...
export async function getMarketsSatsData(): Promise<MarketsSatsData> {
  ...
  if (!btcUsd || btcUsd <= 0) {
    if (marketsCache) return { ...marketsCache.data, stale: true }
