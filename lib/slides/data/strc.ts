...
export async function getStrcSlideData(): Promise<StrcData> {
  const now = Date.now()
  if (strcCache && now - strcCache.timestamp < STRC_CACHE_DURATION_MS) return strcCache.data
  ...
  try {
    ...
    if (strcCache) return strcCache.data
    return emptyStrc()
  }
}
...
export async function getSataSlideData(): Promise<SataData> {
  const now = Date.now()
  if (sataCache && now - sataCache.timestamp < SATA_CACHE_DURATION_MS) return sataCache.data
  ...
  try {
    ...
    if (sataCache) return sataCache.data
    return emptySata()
  }
}
...
async function fetchStrategyStrc() {
  ...
  const row = rows.find((item) => isObject(item) && stringValue(item.company) === "STRC") ?? rows[0]
  if (!isObject(row)) throw new Error("Strategy STRC response missing row")
  return row
}
...
async function fetchStrategyTrackerAsst() {
  ...
  if (!isObject(latest)) throw new Error("StrategyTracker latest response invalid")
  const version = stringValue(latest.version)
  if (!version) throw new Error("StrategyTracker latest response missing version")
  const payload = await fetchJson(`${base}/ASST.v${version}.json`, 20_000)
  if (!isObject(payload)) throw new Error("StrategyTracker ASST response invalid")
  return payload
}
...
function normalizeStrc(row: JsonObject, btcUsd: number): StrcData {
  ...
  const monthlyDiv = latestCashDividend(row.dividendHistory) ?? (annualDiv > 0 ? annualDiv / 12 : 0)
  ...
  if (pffCor != null) correlations.pff = pffCor
  ...
  if (sharpeRatio != null) metrics.sharpeRatio = sharpeRatio
  if (annualizedVolatility != null) metrics.annualizedVolatility = annualizedVolatility
  if (vwap1mo != null) metrics.vwap1mo = vwap1mo
  if (mstrPrice != null) metrics.mstrPrice = mstrPrice
...
function normalizeSata(payload: JsonObject, btcUsd: number): SataData {
  ...
  if (!metrics) return { ...emptySata(), btc: { price: btcUsd } }
