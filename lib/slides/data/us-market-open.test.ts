...
describe('getUsMarketOpenData', () => {
    ...
    it('normalizes US index futures from the configured API', async () => {
        ...
        vi.stubGlobal(
            'fetch',
            vi.fn<typeof fetch>().mockResolvedValueOnce(jsonResponse(marketPayload)),
        );
        ...
        vi.stubGlobal(
            ...
            vi.fn<typeof fetch>().mockResolvedValueOnce(
                jsonResponse({
                    success: true,
                    data: [
                        { symbol: 'SPY', priceUSD: 620.2, changeUSD: 1.4, changePercentUSD: 0.23 },
                    ],
                }),
            ),
        );
    ...
    it('calculates market phases in New York time', async () => {
        ...
        vi.stubGlobal(
            'fetch',
            vi.fn<typeof fetch>().mockResolvedValue(jsonResponse(marketPayload)),
        );
