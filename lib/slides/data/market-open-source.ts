...
async function fetchMarketProviderItems(config: MarketOpenConfig): Promise<MarketProviderResult> {
    ...
    if (providerUrl) {
        return {
            ...
            source:
                process.env[`${config.envPrefix}_MARKET_DATA_PROVIDER`] || 'Configured market API',
            provider: 'configured',
        };
    }
...
async function fetchProviderMarketItems(
    ...
    if (!response.ok) {
        throw new Error(`${config.marketName} market data error: ${response.status}`);
    }
    ...
    if (envelope.success === false) {
        throw new Error(`${config.marketName} market data success=false`);
    }
...
async function fetchStooqMarketItems(config: MarketOpenConfig): Promise<MarketItem[]> {
    ...
    if (symbols.length === 0) {
        throw new Error(`${config.marketName} has no Stooq symbols`);
    }
    ...
    const results = await Promise.allSettled(
        symbols.map(async (symbol) => {
            ...
            if (!response.ok) {
                throw new Error(`Stooq ${symbol} error: ${response.status}`);
            }
...
function parseStooqQuote(symbol: string, csv: string): MarketItem | null {
    ...
    if (!rawSymbol || rawSymbol.toUpperCase() === 'N/D') {
        return null;
    }
    const open = parseFiniteNumber(openRaw);
    const close = parseFiniteNumber(closeRaw);

    if (open === null || open <= 0 || close === null || close <= 0) {
        return null;
    }
